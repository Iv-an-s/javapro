...
public class PaymentController {
    ...
    public PaymentExecutorResponseDto executePayment(@RequestBody PaymentDto payment) {
        logger.info("PaymentController#executePayment: получен запрос с PaymentDto: " + payment);
        PaymentExecutorResponseDto responseDto = productService.executePayment(payment.getUserId(), payment.getProductId(), payment.getAmount());
        logger.info("PaymentController#executePayment: запрос обработан. Возвращаем PaymentExecutorResponseDto: " + responseDto);
        return responseDto;
    }
